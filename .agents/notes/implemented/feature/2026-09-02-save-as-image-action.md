...
**The canvas is measured, not trusted.** `html-to-image` rasterizes through a `foreignObject` whose block-margin handling differs from the live flex layout, so a tall response draws a few percent taller than its DOM box — measured at ~44px of drift on a 3161px response — and a canvas sized to the box clips the last lines, visible as the final line cut down to its top sliver. The render therefore sizes the canvas to the box plus tail headroom (5% of the height, 128px floor), reads back the drawn pixels to find the last row that differs from the background color, and crops to the content plus the node's own padding, so the export frames exactly like the live widget and stays correct under any future re-layout drift. The library's canvas-dimension guard may downscale oversized renders; every offset follows the canvas' real scale, not the requested ratio.

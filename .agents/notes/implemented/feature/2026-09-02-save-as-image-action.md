...
**Rendering is `html-to-image`, bundled into the client bundle.** The render goes through `toCanvas` at `pixelRatio: 2`; responses tall enough to blow the canvas budget retry once at `pixelRatio: 1`, and a second failure propagates into the row's failure feedback rather than being swallowed. Fonts are pre-resolved with `getFontEmbedCSS` and handed to the render as `fontEmbedCSS`; if that pre-pass throws, the render degrades to `skipFonts: true` — the issue explicitly asks that a font-embedding failure cost fidelity, not the export. The package is an ordinary npm dependency bundled by esbuild, while `@deepseek-ai/*` stays external and resolves through the host ModuleLoader at runtime, exactly like the feedback package's primitives dependency.

**The export paints the active skin's background.** `toCanvas` defaults to a transparent canvas, and the captured node's opaque layers do not include the page background — that sits on ancestors the clone does not carry. Under a dark skin the result was light text on a transparent PNG, which reads as invisible once pasted onto a white surface. The render resolves `--dsw-alias-bg-base` with `getComputedStyle` on the captured node itself — the custom property inherits from wherever the active theme applied it — and passes the value as `backgroundColor` on both render attempts; an empty resolution omits the option, preserving the previous behavior outside a skinned surface. The skin catalog defines the variable for every shipped skin, so the catalog remains the single owner of theme colors.

**The canvas is measured, not trusted.** `html-to-image` rasterizes through a `foreignObject` whose block-margin handling differs from the live flex layout, so a tall response draws a few percent taller than its DOM box — measured at ~44px of drift on a 3161px response — and a canvas sized to the box clips the last lines, visible as the final line cut down to its top sliver. The render therefore sizes the canvas to the box plus tail headroom (5% of the height, 128px floor), reads back the drawn pixels to find the last row that differs from the background color, and crops to the content plus the node's own padding — also widened by one padding on each side, painted with the same skin background, so the image frames and breathes exactly like the live widget and stays correct under any future re-layout drift. The library's canvas-dimension guard may downscale oversized renders; every offset follows the canvas' real scale, not the requested ratio.

...
- 三个 surface 默认列出 V4.1 Flash 条目。最初的临时 id 内嵌下线日期；
  2026-09-11（见
  [2026-09-11-dsh-0.1.2-rc.1-upgrade](../process/2026-09-11-dsh-0.1.2-rc.1-upgrade.md)）
  该条目被替换为官方 id `deepseek-flash` 而非删除，因为钉版运行时线
  （0.1.2-rc.1）的 `DEFAULT_MODELS` 仍是三个模型——待原生内置
  `deepseek-flash` 的运行时（0.1.5-rc.1）可达时再整体删除这些层。

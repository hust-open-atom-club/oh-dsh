...
- 更新态渲染已随入口交付,并且**纯粹由快照驱动**:经
  `DesktopBridge.aboutUpdate`(状态推送 + 初始快照)收到的状态里,只有报告
  可安装更新时才显示入口——idle / not-available / checking / error,以及
  管理器上报 `unsupported`(dev / 非打包运行、不支持平台的打包版)时全部保持
  隐藏;available / downloading / downloaded 时显示图标并带红点。
  `unsupported` 管理器报告的是"没有可安装内容",隐藏它可以让正式版与 dev
  表现一致,不出现仅开发态才有的入口。**没有 dev 模拟**:无快捷键、无
  localStorage 覆盖,正式版与 dev 走同一条代码路径,任何测试性出口都不会漏进
  发布构建。

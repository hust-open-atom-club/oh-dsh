...
- 多路复用器切换标签页后返回仍可能留下堆叠的旧帧：窗格隐藏期间排队
  的输出重放，渲染器的帧锚点会与物理屏幕脱同步（zellij + v0.10.0 实
  测；同族问题：dsh-TUI#490）。渲染器自带的恢复手段是 Ctrl+L
  （`redraw` 动作 → `forceRedraw()`），会清屏并原子重绘；usage 文档在
  多路复用器默认说明旁写明。根因修复属于上游。

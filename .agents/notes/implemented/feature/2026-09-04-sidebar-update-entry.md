...
- Update-state rendering ships with the entry and is purely snapshot-driven
  ...
  entry stays hidden unless the snapshot reports an installable update —
  idle / not-available / checking / error and an `unsupported` manager
  (dev / non-packaged runs and unsupported platforms) all stay hidden, and
  available / downloading / downloaded show the icon with the red badge.
  Because an unsupported manager reports nothing installable, hiding it
  keeps packaged and dev runs consistent, with no dev-only affordance.
  There is no dev simulation: no shortcut, no localStorage override, so
  packaged and dev runs follow the same code path and nothing can leak a
  testing affordance into a release build.

...
**画布靠量测，不靠信任。** `html-to-image` 经由 `foreignObject` 光栅化，其块级 margin 的处理与 live 的 flex 布局不同，长回复画出来的内容会比 DOM 盒子高几个百分点——在 3161px 的回复上实测漂移约 44px——按盒子尺寸定画布就会切掉最后几行，表现为最后一行只剩上沿。因此渲染时画布在盒子高度之外追加尾部余量（高度的 5%，下限 128px），然后分条带读回绘制像素、找到最后一行与背景色不同的位置，按「内容 + 节点自身 padding」裁切，并在左右各加一份同样的 padding、涂上同一皮肤背景色，导出的四边留白与 live 控件完全一致。若内容一直画到画布最底行，说明余量被漂移吃光，此时余量翻倍并重新渲染，把失败约束成有界的重试而不是静默裁尾。库的画布尺寸守卫可能把超大渲染整体缩小；所有偏移都跟随画布的真实比例，而不是请求的像素比。
...
捕获保真度继承 `html-to-image` 的局限。渲染器画在捕获子树样式表之外的内容、或无法内联的资源，在 PNG 里可能不一样；超长回复会耗掉像素比重试，两种像素比都失败回复就是导不出。量测裁切也有自己的边界：末尾与背景色差异在容差之内的像素（极淡的分隔线、颜色贴近皮肤底色的 callout 边框）会被当作背景，其最后几行随留白一起被裁掉；翻倍余量只在增长上限内约束漂移，超出上限的病态重排仍会丢尾部。横向溢出按屏幕所见导出：可滚动的代码块与表格保持屏幕上的裁切，而不是展开全部内容。由于图片在本地内联，不会上传任何东西——但内联也意味着渲染过程可能去取页面本身已加载过的同文档资源（图片、字体）。

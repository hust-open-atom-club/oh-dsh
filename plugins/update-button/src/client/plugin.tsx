...
function SidebarUpdateButton({ t, ui }: SidebarUpdateButtonProps): JSX.Element | null {
  ...
  /** Whether the pointer is over the button right now. */
  const isHovered = (): boolean => {
    const target = buttonRef.current
    return target !== null && target.matches(':hover')
  }

  /** Whether the button just received keyboard focus (e.g. Tab). */
  const isKeyboardFocus = (): boolean => Date.now() - lastKeyRef.current < 800
  ...
  // Focus alone must not pop the bubble: after the update window closes the
  // main window refocuses the button, which would otherwise leave a tooltip
  // stuck on screen while the pointer is elsewhere. Show only for real hover
  // or keyboard focus (Tab); the button always carries its aria-label, so
  // assistive tech is unaffected.
  const onFocusShow = (): void => {
    if (!isHovered() && !isKeyboardFocus()) return
  ...
  useEffect(() => {
    ...
    // Keyboard focus (Tab) may legitimately show the tooltip; remember the
    // last key press so onFocus can distinguish it from a window-refocus.
    const onKeyDown = (): void => { lastKeyRef.current = Date.now() }
    // When this window regains focus (e.g. the update window closes) with the
    // pointer elsewhere, never leave a bubble behind.
...
function mountEntry(
  ...
  /**
   * Reveal a rail entry that materialized while the sidebar is already
   * collapsed. The collapsed rail paints its entry at opacity: 0 so a live
   * fold commit can never flash it; onFold() adds the slide-in entry class,
   * but only when a button already exists at that moment. An entry that turns
   * visible after the fold — a cold mount, or an update check completing from
   * About or the update window — is created by React later, and a synchronous
   * query at host creation runs before the previously hidden component has
   * rendered, so nothing would ever show it until the next expand/collapse.
   * Give it the same one-shot slide-in as a live fold instead: nothing is
   * ever painted in place first (an inline opacity would sit through the
   * animation's CSS delay and read as a drop followed by the slide), so every
   * rail entry enters exactly once, from the rail-right edge.
   */
  const revealRailEntry = (): void => {
    // Read the frame attribute, not prevCollapsed: reveal must hold whenever
    // the sidebar is collapsed, including entries born before this tracker
    // ever observed a fold (prevCollapsed only dedupes fold transitions).
    const frame = document.querySelector<HTMLElement>('.oh-dsh-desktop-frame')
    if (frame?.hasAttribute('data-sidebar-collapsed') !== true) return
    const entry = railEntry()
    if (entry === null) return
    // Already shown — the entry class from a live fold or an earlier reveal
    // (running or finished with its forwards fill) owns the entrance.
    if (entry.classList.contains('oh-dsh-sidebar-update-rail-enter')) return
    // The slide-in must start from the CSS opacity: 0 base; clear any
    // leftover inline style so the entrance never begins from a painted
    // state, then schedule the class like onFold() does.
  ...
  const load = (): void => {
    ...
    if (railHost === null || !railHost.isConnected) {
      ...
      // Host creation cannot reveal the entry here: React renders the button
      // asynchronously, so it does not exist yet — revealRailEntry() at the
      // end of this load (and on later reloads) is what shows it.
    }
    ...
    // Run last: any rail entry present now — mounted, remounted, or born
    // since the last reload — gets its collapsed-state reveal.
    revealRailEntry()
  }
